...
int main(int argc, char** argv)
{
    ...
    int force_same=0;
    if (argc > 1) {
        force_same = atoi(argv[1]);
    } else {
        wlog("usage: test-samecen forcecen?\n");
        exit(EXIT_FAILURE);
    }
    gmix.maxiter=1000;
    gmix.tol = 1.e-6;
    ...
    image = image_read_text("/astro/u/esheldon/tmp/timage-sky-noisy-samecen.dat");
    ...
    //gvec->data[0].row -= 0.5;
    //gvec->data[0].col += 0.5;
    //gvec->data[1].row -= 0.35;
    //gvec->data[1].col -= 0.25;
